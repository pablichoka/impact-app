        ...
        return Theme(
          ...
          data: theme.copyWith(
            colorScheme: theme.colorScheme.copyWith(
              primary: theme.colorScheme.secondary, // header background color - using secondary as a base for selected day
              onPrimary: theme.colorScheme.primary, // header text color (e.g., "SELECT DATE") - making it black as per selected day text
              surface: theme.colorScheme.surface, // main dialog background
              onSurface: theme.colorScheme.tertiary, // default text color for days, month/year
            ),
            // specific styling for the date picker
            datePickerTheme: DatePickerThemeData(
              headerBackgroundColor: theme.colorScheme.secondary, // header background
              headerForegroundColor: theme.colorScheme.primary, // header text (e.g. "SELECT DATE", selected date display)
              backgroundColor: theme.colorScheme.surface, // background of the calendar part
              // day text style
              ...
              todayForegroundColor: WidgetStateProperty.all(theme.colorScheme.primary), // text color for today
              todayBackgroundColor: WidgetStateProperty.all(theme.colorScheme.onSurfaceVariant),
              ...
              dayForegroundColor: WidgetStateProperty.resolveWith<Color?>((Set<WidgetState> states) {
                if (states.contains(WidgetState.selected)) {
                  return theme.colorScheme.primary; // text color for selected day
                }
                return theme.colorScheme.tertiary; // default text color for other days
              }),
              dayBackgroundColor: WidgetStateProperty.resolveWith<Color?>((Set<WidgetState> states) {
                if (states.contains(WidgetState.selected)) {
                  return theme.colorScheme.secondary; // background for selected day
                }
            ...
            textButtonTheme: TextButtonThemeData(
              style: TextButton.styleFrom(
                backgroundColor: theme.colorScheme.secondary, // button background color
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                TextButton(
                  ...
                  child: Text('Elegir', style: TextStyle(color: theme.colorScheme.primary)),
                ),
              ],
            ),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded( // text takes available space on the left
                  child: Text(_selectedTime == null
                      ? 'Seleccionar hora'
                      : 'Hora: ${_selectedTime!.format(context)}'),
                ),
                const SizedBox(width: 8), // spacing between text and dropdown
                Expanded( // dropdown takes available space on the right
                  child: DropdownButtonFormField<TimeOfDay>(
                    decoration: InputDecoration(
                      ...
                      filled: true,
                      fillColor: theme.colorScheme.secondary, // light background for dropdown
                    ...
                    items: _availableTimes.map((TimeOfDay time) {
                      return DropdownMenuItem<TimeOfDay>(
                        value: time,
                        child: Text(time.format(context), style: TextStyle(color: theme.colorScheme.primary)), // changed text color to primary
                      );
                    }).toList(),
                    ...
                    hint: Text('Elegir', style: TextStyle(color: theme.colorScheme.primary),), // changed hint
                  ),
                ),
              ],
            ),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (_selectedDate != null && _selectedTime != null) {
                          ...
                          Navigator.of(context).pop(finalDateTime); // pass the selected datetime back
                        } else {
